    ...
    try {
      if (mode === 'login') {
        ...
        if (!email.trim() || !email.includes('@')) {
          throw new Error('Укажите корректный адрес email');
        }
        ...
        await register(email, password, displayName, specialization);
        ...
        setSuccessMsg('Пароль успешно обновлен! Теперь вы можете войти.');
        setTimeout(() => {
          setMode('login');
          setPassword('');
          setConfirmPassword('');
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="text-xs font-medium text-slate-300">
                {mode === 'forgot_password' ? 'Новый пароль *' : 'Пароль *'}
              </label>
              {mode === 'login' && (
                <button
                  type="button"
          ...
          >
            {isSubmitting ? (
              ...
              <>
                <span>
                  {mode === 'login' && 'Войти в FREELA'}
                  {mode === 'register' && 'Создать рабочее пространство'}
                  {mode === 'forgot_password' && 'Обновить пароль'}
                </span>
                <ArrowRight size={15} />
              </>
            )}
